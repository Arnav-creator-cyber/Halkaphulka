...
function getSettings() {
  try {
    ...
    // Structural Safety Net: Ensure the parsed data contains a menu key before serving it
    if (!data || !data.menu) {
      console.warn("⚠️ Settings file was malformed or incomplete. Recovering with default settings.");
...
function saveSettings(settings: any) {
  // Never save empty or corrupted configs to disk
  if (!settings || !settings.menu) {
    console.error("❌ Refusing to save invalid kitchen settings payload to disk.");
    return;
  }
...
app.get("/api/gsheets", (req, res) => {
  ...
  if (type === "updateStatus") {
    ...
    return res.status(404).json({ error: `Order at row ${rowIndex} not found` });
  }
...
// Update order core fields from Admin bottom sheet
...
// Refactored Shared Review Endpoint Function
...
// Route assignments cleanly utilizing the handler function without duplicates
...
// POST Webhook pipeline simulator
